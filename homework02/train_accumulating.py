...
parser.add_argument("--num_checkpoint_parts", help="Number of training epochs",
                    type=int)
parser.add_argument("--num_forward_passes", help="Number of training epochs",
                    type=int, default=1)
...
def train_model(model, loaders, n_epochs, checkpoint_path):
    ...
    for epoch in range(n_epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            loss.backward()
            current_batch_times.append(time.perf_counter() - batch_start_time)

            if not (i + 1) % args.num_forward_passes:
                optimizer.step()
                optimizer.zero_grad()
                batch_time.append(sum(current_batch_times))
                current_batch_times = []
